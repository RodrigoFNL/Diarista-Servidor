...
public class WorkBusiness extends BasicBusiness<Work> 
{
	...
	public Double getValueWork(Work work) 
	{
		List<TaskAmmount>  tasks = work.getTasks();
		Double retorno = 0.0;

		for (TaskAmmount task : tasks) 
		{
			retorno += task.getAmmount() * task.getTask().getValue();
		}

		return retorno < Work.VALOR_MINIMO ? Work.VALOR_MINIMO : retorno;
	}
}
